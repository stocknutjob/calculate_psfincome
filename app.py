...
def calculate_annual_pension_tax(payout_under_limit: float, other_pension_income: int, other_comprehensive_income: int, current_age: int) -> dict:
    ...
    # 1,500만원 초과 시 종합과세와 분리과세(16.5%) 중 유리한 쪽 선택
    else:
...
def display_tax_choice_summary(simulation_df):
    ...
    # 1500만원 초과로 과세 방식 선택이 일어나는 데이터만 필터링
    choice_df = simulation_df[simulation_df['선택'] != '저율과세'].copy()
...
if 'user_input' not in st.session_state:
    st.session_state.user_input = UserInput(
        ...
        other_pension_income=0, other_comprehensive_income=0,
        income_level=INCOME_LEVEL_LOW, contribution_timing='연말'
    )
...
with st.sidebar:
    ...
    ui.pre_retirement_return = st.number_input("은퇴 전 수익률", -99.9, 99.9, format="%.1f", step=0.1, on_change=reset_calculation_state, disabled=not is_direct_input, help=help_text_return)
    ui.post_retirement_return = st.number_input("은퇴 후 수익률", -99.9, 99.9, format="%.1f", step=0.1, on_change=reset_calculation_state, disabled=not is_direct_input, help=help_text_return)
    ui.inflation_rate = st.number_input("예상 연평균 물가상승률", -99.9, 99.9, format="%.1f", step=0.1, on_change=reset_calculation_state)
    
    st.subheader("연간 납입액 (원)")
    st.info(
        f"연금계좌 총 세액공제 한도: 연 900만원\n"
        f"계좌 총 납입 한도: 연 {MAX_CONTRIBUTION_LIMIT/10000:,.0f}만원\n"
        f"(단, 이 계산기는 연금저축 단독 한도인 연 {PENSION_SAVING_TAX_CREDIT_LIMIT/10000:,.0f}만원까지만 자동 계산에 반영합니다.)"
    )
    ...
    ui.other_non_deductible_total = st.number_input("그 외, 세액공제 받지 않은 총액", 0, step=100000, on_change=reset_calculation_state, help="ISA 만기 이전분 등 납입 기간 동안 발생한 비과세 원금 총합을 입력합니다.")
    ...
    ui.other_pension_income = st.number_input("국민연금 등 다른 연금 소득 (연간 세전)", 0, step=500000, on_change=reset_calculation_state)
    ui.other_comprehensive_income = st.number_input("임대, 사업 등 그 외 종합소득금액", 0, step=1000000, on_change=reset_calculation_state, help="부동산 임대소득 등 사업소득금액(총수입-필요경비)을 입력하세요.")
